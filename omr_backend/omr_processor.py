...
class OMRProcessor:
    def __init__(
        ...
        try:
            ...
            logger.debug("torch patch not applied / torch not available.")
    ...
    def warp_omr(self, image_path: str) -> np.ndarray:
        ...
        return self.warp_omr_from_array(img)

    def warp_omr_from_array(self, img: np.ndarray) -> np.ndarray:
        if img is None or not isinstance(img, np.ndarray):
            raise ValueError("warp_omr_from_array expects a valid numpy.ndarray image (BGR).")
        ...
        if len(squares) >= 4:
            ...
            raise ValueError("Not enough markers detected for provided image (needs >=4 markers).")
    ...
    def evaluate_bubbles(self, warped: np.ndarray, answer_key: dict, roi_r: int = 8) -> Tuple[dict, int]:
        ...
        for q in self.TEMPLATE_POS:
            ...
            # ✅ Use string key for answer comparison
            # ✅ Compare zero-indexed answers
            if str(qid) in answer_key and filled == answer_key[str(qid)]:
                score += 1
    ...
    def process_image_array(self, img_array: np.ndarray, answer_key: dict) -> dict:
        start = time.time()
        try:
            warped = self.warp_omr_from_array(img_array)
            detected, score = self.evaluate_bubbles(warped, answer_key)
            elapsed = time.time() - start
            return {"score": int(score), "detected": detected, "time_sec": round(elapsed, 3)}
        except Exception as e:
            logger.exception("Error processing image array: %s", e)
            return {"error": str(e)}
